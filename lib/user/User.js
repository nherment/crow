...
Client.prototype.createSession = function(userId, callback) {
  this.pg.createSession(userId, (err, result) => {
    if(err) {
      logger.error(err)
      callback(err, null)
 ...
 * - not_found
 */
Client.prototype.authenticate = function(email, password, callback) {
  ...
  this.pg.fetchUser(email, (err, result) => {
    if(err) {
      ...
      if (!user.active) {
        err = new Error(`Account is locked. Please contact an administrator. [${email}]`)
      ...
      crypto.verify(password, hashedPassword, (correctPassword) => {
        if(correctPassword) {
          ...
          this.createSession(user.id, (err, session) => {
...
Client.prototype.fetchUserFromSession = function(sessionToken, callback) {
  //logger.info('fetchUserFromSession', sessionToken)
  this.pg.fetchUserFromSession(sessionToken, function(err, result) {
    if(err) {
      callback(err, undefined)
    } else if(!result || result.length < 1) {
      logger.warn('fetchUserFromSession: invalid session', sessionToken)
      callback(null, null)
    } else {
      let user = result[0]
      if(user.active) {
        ...
        logger.warn('fetchUserFromSession: user locked', sessionToken)
...
Client.prototype.updatePassword = function(userID, oldPassword, newPassword, callback) {
