...
function pullUserFromSession(app) {
  app.use(function(req, res, next) {
    if(req.session && req.session.token && !req.user) {
      user.fetchUserFromSession(req.ip, 'web', req.session.token, function(err, user, token, expiry) {
        if(err) {
          res.error(err)
        } else if(user) {
          req.user = user
...
function requireAuth(app) {
  app.use(function(req, res, next) {
    if(req.session && req.session.token && !req.user) {
      user.fetchUserFromSession(req.ip, 'web', req.session.token, function(err, user) {
        if(err) {
          res.error(err)
        } else if(user) {
          req.user = user
